...
const Preview = () => {
    ...
    const fetchAnnouncement = async () => {
      ...
      try {
        ...
        const { data: communityData, error: communityError } = await supabase
          ...
          .select('community_id, views, clicks')
        ...
        if (communityData && communityData.length > 0) {
          ...
          for (let i = 6; i >= 0; i--) {
            ...
            lastWeek.push({
              ...
              views: Math.floor(Math.random() * 100), // Placeholder - replace with real data
        ...
        if (communityIds.length > 0) {
          ...
          const communityCost = enhancedCommunities.reduce((sum: number, community: any) => 
  ...
  const renderPreviewTab = () => (
    <Card className="border border-nexus-border-subtle bg-nexus-background-card/70 backdrop-blur-md">
      <CardHeader>
      ...
      <CardContent className="space-y-4">
        <div className="space-y-4">
          ...
          {announcement.media_url && (
            <div className="relative rounded-md overflow-hidden border border-nexus-border-subtle">
          ...
          {communities.length > 0 ? (
            <ul className="list-disc pl-5">
              {communities.map((community) => (
                <li key={community.id} className="text-nexus-text-muted">
            ...
            <p className="text-nexus-text-muted">No communities selected.</p>
          )}
        </div>
      ...
      <CardFooter className="flex justify-between">
        ...
        {announcement.status !== 'PUBLISHED' && announcement.status !== 'ACTIVE' && (
          <Button 
        ...
        <div className="mt-6">
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-white">
              ...
              <tbody>
                {communities.map((community) => (
                  <tr key={community.id} className="border-b border-nexus-border-subtle">
