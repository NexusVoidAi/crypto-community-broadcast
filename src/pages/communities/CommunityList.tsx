...
const CommunityList: React.FC = () => {
  ...
  const fetchCommunities = async () => {
    ...
    try {
      ...
      } = await supabase.from('communities').select('*').eq('approval_status', 'APPROVED').order('created_at', {
        ascending: false
      });
  ...
  const fetchTotalEarnings = async () => {
    try {
      const {
        data,
        error
      } = await supabase.from('community_earnings').select('amount').eq('currency', 'USDT');
  ...
  const filteredMyCommunities = myCommunities.filter(community => community.name.toLowerCase().includes(searchQuery.toLowerCase()) || community.description?.toLowerCase().includes(searchQuery.toLowerCase()));
  ...
  const renderBadgeList = (items: string[] | undefined, icon: React.ReactNode) => {
    ...
    return <div className="flex flex-wrap gap-1 mt-2">
        {items.map((item, index) => <Badge key={index} variant="outline" className="bg-crypto-darkgray/50 text-xs font-normal py-0 h-5 border-border/30 flex items-center text-white">
            {icon}
      ...
      <div className="container mx-auto px-4 py-8">
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-6 gap-4">
          ...
          <Link to="/communities/create">
            <Button className="text-crypto-dark bg-crypto-violet">
        ...
        <div className="relative mb-6">
          ...
          <Input placeholder="Search communities..." className="pl-10 bg-crypto-darkgray/50 border-border/50 text-white placeholder:text-white/60" value={searchQuery} onChange={e => setSearchQuery(e.target.value)} />
        </div>
        ...
        <Card className="border-border/50 bg-crypto-darkgray/50">
          <CardContent className="p-6">
            ...
            {isLoading ? <div className="flex justify-center py-12">
              ...
              </div> : filteredMyCommunities.length > 0 ? <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {filteredMyCommunities.map(community => <Link to={`/communities/${community.id}`} key={community.id}>
                    <Card className="h-full hover:border-crypto-blue/50 transition-colors border border-border/50 bg-crypto-darkgray/50 backdrop-blur-md hover:shadow-lg hover:shadow-crypto-blue/10 hover:-translate-y-1 transition-all duration-300">
                      <CardContent className="p-6">
                        ...
                        <Badge variant="outline" className={`mt-3 ${community.approval_status === 'APPROVED' ? 'bg-crypto-green/10 text-crypto-green border-crypto-green/20' : community.approval_status === 'PENDING' ? 'bg-yellow-500/10 text-yellow-500 border-yellow-500/20' : 'bg-red-500/10 text-red-500 border-red-500/20'}`}>
